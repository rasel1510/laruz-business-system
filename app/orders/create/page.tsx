...
const Field = ({
  ...
  <div className={`flex flex-col gap-1 ${className}`}>
    <label className="text-[11px] font-medium uppercase tracking-wider text-white">
      {label}
      {optional && <span className="ml-1 text-white/60 normal-case tracking-normal">(optional)</span>}
    </label>
    {children}
  </div>
);
...
const baseInput =
  "w-full bg-[#0d1117] border border-[#2a3547] rounded-lg px-3 h-10 text-sm text-white placeholder:text-white/40 focus:outline-none focus:border-blue-500/70 hover:border-[#3a4557] transition-colors";
...
function Dropdown({ value, options, onChange }: { value: string; options: string[]; onChange: (v: string) => void }) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <span className={value ? "text-white" : "text-white/40"}>{value || "Select…"}</span>
        <ChevronDown className={`h-3.5 w-3.5 text-white transition-transform shrink-0 ${open ? "rotate-180" : ""}`} />
      </button>
      {open && (
        <div className="absolute z-50 mt-1 w-full bg-[#111827] border border-[#2a3547] rounded-lg shadow-xl overflow-auto max-h-48">
          {options.map((opt) => (
            <button
              ...
              className={`w-full text-left px-3 py-2 text-sm transition-colors ${value === opt ? "bg-blue-600/20 text-blue-400" : "text-white hover:bg-[#1f2937]"}`}
            >
              {opt}
            </button>
          ))}
  ...
  return (
    <div ref={ref} className="relative">
      <div className="relative">
        <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-white" />
        <input
          type="text"
          ...
          {filtered.map((p) => (
            ...
            >
              ...
              <div className="flex items-center gap-3 mt-0.5">
                <span className="text-white text-xs">৳{p.retailPrice.toLocaleString()}</span>
      ...
      {open && q && filtered.length === 0 && (
        <div className="absolute z-50 mt-1 w-full bg-[#111827] border border-[#2a3547] rounded-lg shadow-xl p-3 text-white text-xs text-center">
          No products match &quot;{q}&quot;
        </div>
      )}
    </div>
  ...
  return (
    <section className="flex-1 overflow-y-auto bg-[#0b0e14] min-h-screen">
      ...
      <div className="sticky top-0 z-10 flex items-center gap-3 border-b border-[#1f2937] bg-[#0b0e14]/95 backdrop-blur-sm px-4 sm:px-6 py-3">
        <Link href="/orders">
          <button type="button" className="h-8 w-8 rounded-lg border border-[#2a3547] flex items-center justify-center text-white hover:text-white hover:border-slate-500 transition-colors">
        ...
        <div className="bg-[#111827] border border-[#1f2937] rounded-xl overflow-visible shadow-xl">
          ...
          <div className="border-b border-[#1f2937]">
            ...
            <div className="flex items-center justify-between px-4 sm:px-5 py-2.5 bg-[#0d1117]/60">
              <span className="text-[10px] font-bold uppercase tracking-widest text-white">Products</span>
              <button
                type="button"
                onClick={addItem}
            ...
            <div className="hidden sm:grid grid-cols-12 gap-2 px-4 sm:px-5 py-1.5 border-t border-[#1f2937]">
              <div className="col-span-5 text-[10px] uppercase text-white">Product</div>
              <div className="col-span-3 text-[10px] uppercase text-white">Unit Price</div>
              <div className="col-span-2 text-[10px] uppercase text-white">Qty</div>
              <div className="col-span-2 text-[10px] uppercase text-white">Total</div>
            </div>
              ...
              {items.map((item) => (
                <div key={item.localId} className="px-4 sm:px-5 py-2.5">
                  ...
                  <div className="hidden sm:grid grid-cols-12 gap-2 items-center">
                    ...
                    <div className="col-span-1 text-white text-sm font-semibold">
                      {(item.unitPrice * item.quantity).toLocaleString()}
                    </div>
                    <div className="col-span-1 flex justify-end">
                      <button type="button" onClick={() => removeItem(item.localId)} disabled={items.length === 1}
                        className="text-white hover:text-red-400 transition-colors disabled:opacity-20">
                  ...
                  <div className="sm:hidden space-y-2">
                    ...
                    <div className="grid grid-cols-3 gap-2 items-center">
                      <div>
                        <p className="text-[10px] text-white mb-1">Unit Price</p>
                        <NumInput value={item.unitPrice} onChange={(v) => updateItem(item.localId, { unitPrice: v })} />
                      </div>
                      <div>
                        <p className="text-[10px] text-white mb-1">Qty</p>
                        <NumInput value={item.quantity} min={1} onChange={(v) => updateItem(item.localId, { quantity: Math.max(1, v) })} />
                      </div>
                      <div>
          ...
          <div className="flex items-center justify-between px-4 sm:px-5 py-3 bg-[#0d1117]/80">
            <span className="text-sm text-white font-medium">Grand Total (COD)</span>
          ...
          <div className="flex items-center justify-end gap-3 px-4 sm:px-5 py-3 border-t border-[#1f2937] bg-[#0d1117]">
            <Link href="/orders">
              <button
                type="button"
                className="h-9 px-5 text-sm rounded-lg border border-[#2a3547] text-white hover:bg-[#1f2937] hover:text-white transition-colors"
              >
                Cancel
              </button>
            </Link>
